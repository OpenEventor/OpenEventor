...
func (h *Handler) ListEvents(c *fiber.Ctx) error {
	db := h.DB.SystemDB()
	rows, err := db.Query(
		`SELECT id, filename, display_name, date, status, created_at
	...
	for rows.Next() {
		var ev models.Event
		if err := rows.Scan(&ev.ID, &ev.Filename, &ev.DisplayName, &ev.Date, &ev.Status, &ev.CreatedAt); err != nil {
...
func (h *Handler) GetEvent(c *fiber.Ctx) error {
	...
	var ev models.Event
	err := db.QueryRow(
		"SELECT id, display_name, date, status, created_at FROM events WHERE id = ?",
		eventID,
	).Scan(&ev.ID, &ev.DisplayName, &ev.Date, &ev.Status, &ev.CreatedAt)
	if err != nil {
...
func (h *Handler) DeleteEvent(c *fiber.Ctx) error {
	...
	// Close the event DB connection, then remove the event record.
